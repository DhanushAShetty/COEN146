// Name: Manpreet Sokhi
// Date: May 20, 2020
// Title: Lab assignment 7: Link state routing
...
int			costs[N][N];
int			distances[N];
int			myid, nodes; // routerId, routerPort
...
int main (int argc, char *argv[]) {
	...
	if ((fp = fopen (argv[3], "r")) == NULL) {
        printf("Was not able to read the file");
		return 1;
	}
	...
	for (i = 0; i < 3; i++) {
	    if (i == 0) {
            pthread_create(&thr1, NULL, receive_info, NULL); // bring back later
	    } else if (i == 1) {
...
void *receive_info (void *arg) {
	...
	while(true) {
	    // recvfrom(sock, new, sizeof(new), 0, sockaddr, sockaddr); // fix later
	    n0 = new[0];
        n1 = new[1];
        n2 = new[2];

        printf("Got the enw update trying to update\n");
...
void *update_costs (void *arg) {
    ...
    while(1) {
        ...
        for(i = 0; i < N; i++) {
            ...
            sendto(sock, info, sizeof(info), 0, (struct sockaddr*) &otheraddr, addrLen);
            printf("sent\n");
        }
    }
}
...
void *run_link_state (void *arg) {
	int	taken[N]; // visited
	int	min, minIndex, spot, count;
	int	i, j;
	int	r;
	int distances[N]; // array of least distances
