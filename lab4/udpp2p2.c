...
//    servAddr1.sin_addr = *((struct in_addr *)host->h_addr);//send to 8.8.8.8
//    //send .....5001
//
//    //Setup the server address to bind using socket addressing structure
//    servAddr2.sin_family = AF_INET;
//    servAddr2.sin_port = htons(5002); //Port 5000 is assigned
//    servAddr2.sin_addr = *((struct in_addr *)host->h_addr); //Local IP address of any interface is assigned (generally one interface IP address)
//    // recv

    //.............................p2
    servAddr1.sin_family = AF_INET;
    servAddr1.sin_port = htons(5002);
    servAddr1.sin_addr = *((struct in_addr *)host->h_addr);//send to 8.8.8.8
    //send .....

    //Setup the server address to bind using socket addressing structure
    servAddr2.sin_family = AF_INET;
    servAddr2.sin_port = htons(5001); //Port 5000 is assigned
    servAddr2.sin_addr = *((struct in_addr *)host->h_addr); //Local IP address of any interface is assigned (generally one interface IP address)
    // recv
    //-----------------------
...
void server() {
    ...
    // server open the source file specified
...
int main() {
    while (1) {
        char input;
        printf("Peer 2: would you like to send (S), receive (R), or quit (Q)? \n");
        scanf("%c", &input);
        if (input == 'S' || input == 's') {
            printf("Peer 2 is sending!\n");
            printf("Enter filename: ");
            scanf("%s", fsrc);
            server();
        } else if (input == 'R' || input == 'r') {
            printf("Peer 2 is receiving\n");
            printf("Enter filename: ");
            scanf("%s", fdest);
            client();
